...
`ifndef SVR_SLAVE_DRIVER__SV
    ...
    task svr_slave_driver::get_and_drive(svr_transfer#(DATA_WIDTH) tr);  
        ...
        else if ((cfg.prt_type == AP_VLD) || (cfg.prt_type == AP_ACK) || (cfg.prt_type == DIRECTIO_AP_VLD) || (cfg.prt_type == DIRECTIO_AP_ACK) || (cfg.prt_type == DIRECTIO_AP_NONE)) begin    // no delay for AP-VLD / AP-ACK protocol
        end
        else
