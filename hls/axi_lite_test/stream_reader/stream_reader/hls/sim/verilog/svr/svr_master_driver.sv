...
`ifndef SVR_MASTER_DRIVER__SV
    ...
    task svr_master_driver::get_and_drive(svr_transfer#(DATA_WIDTH) tr);   
        ...
        if(!$cast(rw_tr, tr)) begin
            if ((cfg.prt_type == AP_HS) || (cfg.prt_type == AXIS) || (cfg.prt_type == AP_FIFO) || (cfg.prt_type == DIRECTIO_AP_HS)) begin
                if (tr.delay > 0 && cfg.spec_cfg != VOLATILE) begin
                    repeat (tr.delay) @(posedge vif.clk);
                end
            end else if ((cfg.prt_type == AP_VLD ) || (cfg.prt_type == AP_ACK) || (cfg.prt_type == AP_NONE) || (cfg.prt_type == DIRECTIO_AP_VLD) || (cfg.prt_type == DIRECTIO_AP_ACK) || (cfg.prt_type == DIRECTIO_AP_NONE)) begin
                // AP_VLD, AP_ACK, AP_NONE no delay
                //vif.ready = 1'b1;
            end else
